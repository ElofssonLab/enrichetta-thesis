...
#path='/home/enrichetta/Desktop/alignment/fasta/'
#path='/media/data/fasta/'
path=sorted(glob.glob(path+'*.sth'))

def variable():
	global dis_annotation, ord_annotation, n_dis_region, n_ord_region, av, disar, ordar, hom_used, hom_original, leno, lend
	dis_annotation={}
	ord_annotation={}
	n_dis_region={}
	n_ord_region={}
...
def entropy_matrix(files):
	...
	alphabet_gap.append('-')
	#alphabet_gap.append('.') # use this if the format of the alignment is a2m
	gap=0
...
def print_result1():
	with open('result.txt','w') as p:
		saveout=sys.stdout
		sys.stdout=p
		print '{0:^12} {1:^15} {2:^12} {3:^12} {4:^10}{5:^15}{6:^10}'.format('ID','n_dis_region','n_ord_region','n_hom_used','<Hd>','<Ho>','av_entropy')
		for l in sorted(dis_annotation):
			try:
			
				print '{0:^12} {1:^15} {2:^12} {3:^12}{4:^12}{5:^12}{6:^12}'.format(l,n_dis_region[l],n_ord_region[l],hom_used[l],dis_annotation[l],ord_annotation[l],av[l])
			except:
				print '{0:^12} {1:^15} {2:^15} {3:^12}{4:^10}{5:^12}{6:^12}'.format(name ,'*', '*', '*', '*', '*', '*')

		p.close()
		sys.stdout=saveout

def print_result2():
	with open('resultwithlen.txt','w') as p:
		saveout=sys.stdout
		sys.stdout=p
